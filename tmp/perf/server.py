...
class Server(object):
  ...
  def __init__(self, node_name):
    ...
    # 获取SSH选项
    ssh_opts = os.environ.get('SSH_OPTS', '').split()
    
    if openssh_config_exist and found_node_name_in_ssh:
      print(host_conf)
      self.ip = host_conf['hostname']
      self.username = getpass.getuser()
      self.remote_cmd = ["ssh"] + ssh_opts + [node_name]
    else:
      self.ip = node_name
      self.username = getpass.getuser()
      self.remote_cmd = ["ssh"] + ssh_opts + [f"{self.username}@{self.ip}"]
  ...
  def remote_get_free_cores_ssh(self, threads):
    ...
    for attempt in range(1, max_attempts + 1):
      ...
      # print(f"Attempt {attempt}: {ssh_cmd_str}")
      ...
      try:
        ...
        if len(result) > 0 and (exit_code is None or exit_code == 0):
          result = ast.literal_eval(result)
          # (free, mem, start, end, server_cores)
          return result
        else:
          print(f"Warning: SSH attempt {attempt} failed for {self.ip}, empty result or non-zero exit code")
          
      except Exception as e:
        print(f"Warning: SSH attempt {attempt} failed for {self.ip}: {e}")
      
      if attempt < max_attempts:
        import time
        time.sleep(2)  # 等待2秒后重试
    
    print(f"Error: All SSH attempts failed for {self.ip}")
